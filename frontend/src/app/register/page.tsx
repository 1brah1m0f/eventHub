...
export default function RegisterPage() {
  ...
  const schema = useMemo(() => z.object({
    ...
    linkedin_url: z.string().url(t('validationUrl')).optional().or(z.literal('')),
  }), [t]);
    ...
    <div className="min-h-[100dvh] flex items-center justify-center px-4 py-10 bg-gray-50">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">{t('joinCommunity')}</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <input {...register('name')} type="text" className={inputClass} placeholder={t('yourName')} />
            ...
            <div>
              ...
              <input {...register('password')} type="password" className={inputClass} placeholder={t('min6Chars')} />
            ...
            <div>
              ...
              <textarea {...register('bio')} rows={2} className={inputClass} placeholder={t('bioPlaceholder')} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                {t('linkedinUrl')} <span className="text-gray-400 font-normal">{t('optional')}</span>
              </label>
        ...
        <p className="text-sm text-center mt-4 text-gray-500">
          ...
          <Link href="/login" className="text-violet-800 font-medium hover:underline">{t('signIn')}</Link>
        </p>
      </div>
    </div>
  );
